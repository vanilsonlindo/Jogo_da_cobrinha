...
public class Snake {
    ...
    bm_body_top_right, bm_body_top_left, bm_body_top_bottom_right, bm_body_bottom_left, bm_tail_right, bm_tail_left,
    bm_tail_up, bm_tail_down;

    private int x,y, length;
    private ArrayList<PartSnake> arrayPartSnake = new ArrayList<>();
    ...
    public Snake(Bitmap bm, int x, int y, int length) {
        ...
        bm_body_top_bottom_right = Bitmap.createBitmap(bm, GameView.sizeOfMap, 0, GameView.sizeOfMap, GameView.sizeOfMap);
        ...
        arrayPartSnake.add(new PartSnake(bm_head_right, x, y));
        for (int i = 1; i < length; i++) {
            arrayPartSnake.add(new PartSnake(bm_body_horizontal, arrayPartSnake.get(i-1).getX() - GameView.sizeOfMap, y));
        }
        arrayPartSnake.add(new PartSnake(bm_tail_right, arrayPartSnake.get(length-2).getX() - GameView.sizeOfMap, y));
    }

    public void draw(Canvas canvas) {
        for (int i = 0; i < length; i++) {
            canvas.drawBitmap(arrayPartSnake.get(i).getBm(), arrayPartSnake.get(i).getX(), arrayPartSnake.get(i).getY(), null);
        }
    ...
    public Bitmap getBm_body_top_bottom_right() {
        return bm_body_top_bottom_right;
    }

    public void setBm_body_top_bottom_right(Bitmap bm_body_top_bottom_right) {
        this.bm_body_top_bottom_right = bm_body_top_bottom_right;
    }
    ...
    public ArrayList<PartSnake> getArrayPartSnake() {
        return arrayPartSnake;
    }

    public void setArrayPartSnake(ArrayList<PartSnake> arrayPartSnake) {
        this.arrayPartSnake = arrayPartSnake;
    }
}
